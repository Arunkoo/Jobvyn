...
import { payment_service_url, useAppData } from "@/context/AppContext";
...
const SubscriptionPage = () => {
  const razorPayLoader = useRazorPay();
  ...
  const handleSubscribe = async () => {
    ...
    const {
      ...
      `${payment_service_url}/api/payment/checkout`,
      {},
    ...
    const options = {
      ...
      name: "JobVyn",
      description:
        "Get noticed by recruiter early + carrer guidance and Resume Analysis",
      order_id: order.id,
      handler: async function (response: any) {
        ...
        try {
          const { data } = await axios.post(
            `${payment_service_url}/api/payment/verifyPayment`,
            {
              razorpay_order_id,
              razorpay_payment_id,
              razorpay_signature,
    ...
    if (!razorPayLoader) console.log("something went wrong with script");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-secondary/30">
      <Card className=" max-w-md w-full p-8 text-center shadow-lg border-2">
        ...
        <div className="mb-6">
          <p className="text-5xl font-bold text-blue-600">₹ 299</p>
